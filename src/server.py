...
async def run_architect_background(prompt: str, model: str):
    ...
    try:
        ...
        tools = [
            {"type": "function", "function": {"name": "AddNode", "description": "Adds a new node to the workflow.", "parameters": {"type": "object", "properties": {"id": {"type": "string"}, "type": {"type": "string", "enum": ["TRIGGER", "AI_AGENT", "TOOL_EXECUTION", "ROUTER", "SWITCH", "PROMPT_INPUT", "KNOWLEDGE", "JSON_PARSER", "JSON_FIELD_EXTRACT", "MERGE", "LOOP_OVER_ITEMS", "VARIABLE_STORE", "FILE_SAVE", "STOP_AND_ERROR", "HTTP_REQUEST", "WAIT", "CODE", "SET", "FILTER", "SORT", "LIMIT", "AGGREGATE", "REMOVE_DUPLICATES", "SPLIT_OUT", "SUMMARIZE", "HTML", "EXECUTE_WORKFLOW", "OUTPUT_DISPLAY"]}, "title": {"type": "string"}, "x": {"type": "integer"}, "y": {"type": "integer"}, "config": {"type": "object", "properties": {"promptText": {"type": "string"}, "modelId": {"type": "string"}, "systemPrompt": {"type": "string"}, "allowedTools": {"type": "array", "items": {"type": "string"}}, "selectedToolName": {"type": "string"}, "httpUrl": {"type": "string"}, "httpMethod": {"type": "string"}, "ruleCondition": {"type": "string"}, "routerMode": {"type": "string"}, "switchRoutes": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "condition": {"type": "string"}}}}, "setFields": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "value": {"type": "string"}, "type": {"type": "string"}}}}, "htmlOperation": {"type": "string"}, "htmlExtractionValues": {"type": "array"}, "summarizeFields": {"type": "array"}, "sortFieldName": {"type": "string"}, "sortOrder": {"type": "string"}, "limitCount": {"type": "integer"}, "mergeMode": {"type": "string"}, "fileName": {"type": "string"}, "waitForAllInputs": {"type": "boolean"}}, "additionalProperties": True}, "required": ["type"]}}}},
        ...
        node_descriptions = """
AVAILABLE MCP TOOLS (for AI_AGENT's allowedTools). Query /api/architect/mcp-tools to see what's available on this server. Use ONLY tools prefixed with "mcp__" from that list.
When building a workflow, the AI_AGENT's allowedTools should be set to the names of MCP tools the agent needs. Example: ["mcp__Brave Search", "mcp__Fetch"]
...
• AI_AGENT: Uses LLM. `modelId` (DEFAULT: "qwen/qwen3.6-35b-a3b" — ALWAYS use this model, do NOT use GPT-4, Claude, or any other model), `systemPrompt`, `allowedTools` (list of "mcp__" tool names).
